...
# From psycopg2
@router.get("/posts-from-psycopg2")
async def get_post():
    cursor.execute(""" SELECT * FROM posts """)
    posts = cursor.fetchall()
    '''
...
# From psycopg2
@router.post("/create-posts-from-psycopg2", status_code=status.HTTP_201_CREATED)
...
# From SQLalchemy ORM
@router.post("/create-post-from-orm", status_code=status.HTTP_201_CREATED)
def create_posts(post: schemas.PostCreate, db:Session = Depends(get_db)):
    new_post = models.Post(**post.dict())
...
# From psycopg2
@router.get("/get-post-by-id-from-psycopg2/{id}")
def get_post(id: int):
    cursor.execute("""SELECT * FROM posts WHERE id = %s """, (id,))
    '''
...
# From orm
@router.get("/get-post-by-id-from-orm/{id}")
def get_post(id: int, db: Session = Depends(get_db)):
    test_post = db.query(models.Post).filter(models.Post.id == id).first()
    if not test_post:
...
# From psycopg2
@router.delete("/delete-post-by-psycopg2/{id}", status_code=status.HTTP_204_NO_CONTENT)
...
# From orm
@router.delete("/delete-post-by-orm/{id}")
def delete_post(id: int, db: Session = Depends(get_db)):
    deleted_post = db.query(models.Post).filter(models.Post.id == id)
    if deleted_post.first() == None:
        conn.rollback()
...
# from psycopg2
@router.put("/update-post-by-psycopg/{id}")
...
# from orm
@router.put("/update-post-by-orm/{id}")
